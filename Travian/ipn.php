...
$notify_email =  "paypal@delviewtravian.com";         //email address to which debug emails are sent to
$DB_Server = "localhost:3306"; //your MySQL Server
$DB_Username = "delviewtra"; //your MySQL User Name
$DB_Password = "delviewtravian12345"; //your MySQL Password
$DB_DBName = "delviewtra_s1"; //your MySQL Database Name
...
//execute query
    ...
    if ($item_name == "30") {
        ...
        //do nothing haxors
            ...
            mysql_query("UPDATE p_players p  SET p.gold_num=gold_num+'30' WHERE p.id='".$item_number."'");
          } 
		
    } elseif ($item_name == "100") {
             if ( $mc_gross != "4.99"){
        //do nothing haxors
         }else{
             mysql_query("UPDATE p_players p  SET p.gold_num=gold_num+'100' WHERE p.id='".$item_number."'");
		}
    } elseif ($item_name == "250") {
 if ( $mc_gross != "9.99"){
        //do nothing haxors
         }else{
             mysql_query("UPDATE p_players p  SET p.gold_num=gold_num+'250' WHERE p.id='".$item_number."'");
	}	
    } elseif ($item_name == "600") {
 if ( $mc_gross != "19.99"){
        //do nothing haxors
         }else{
            mysql_query("UPDATE p_players p  SET p.gold_num=gold_num+'600' WHERE p.id='".$item_number."'");
	}	
    }
