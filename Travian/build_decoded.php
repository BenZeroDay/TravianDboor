...
class GPage extends VillagePage
{
    ...
    public function onLoadBuildings( $building )
    {
        ...
        if ( $building['item_id'] == 23 && 0 < $building['level'] )
        {
            ...
            $this->crannyProperty['totalSize'] += $this->crannyProperty['buildingCount'];
        }
    }
    ...
    public function handleBlacksmithArmoury( )
    {
        ...
        if ( isset( $_GET['a'], $_GET['k'] ) && $_GET['k'] == $this->data['update_key'] && !isset( $this->queueModel->tasksInQueue[$this->troopsUpgradeType] ) && isset( $this->troopsUpgrade[intval( $_GET['a'] )] ) && !$this->isGameTransientStopped( ) && !$this->isGameOver( ) )
        {
    ...
    public function handleMainBuilding( )
    {
        ...
        else if ( isset( $_GET['qid'] ) && is_numeric( $_GET['qid'] ) && isset( $_GET['k'] ) && $_GET['k'] == $this->data['update_key'] && isset( $_GET['d'] ) && isset( $this->queueModel->tasksInQueue[QS_BUILD_DROP] ) && !$this->isGameTransientStopped( ) && !$this->isGameOver( ) )
        {
   ...
   public function _canCancelWarTask( $taskType, $taskId )
    ...
    public function _getCurrentNumberFor( $troopId, $item )
    {
        ...
        if ( isset( $this->queueModel->tasksInQueue[$this->troopsUpgradeType][$item], $this->queueModel->tasksInQueue[$this->troopsUpgradeType][$item] ) )
        {
    ...
    public function _getMaxTrainNumber( $troopId, $item )
    {
        ...
        if ( $troopId == 99 )
        {
            ...
            if ( isset( $this->queueModel->tasksInQueue[$this->troopsUpgradeType][$this->buildProperties['building']['item_id']], $this->queueModel->tasksInQueue[$this->troopsUpgradeType][$this->buildProperties['building']['item_id']] ) )
            {
    ...
    public function getActionText2( $neededResources )
    {
        ...
        if ( !$this->isResourcesAvailable( $neededResources ) )
        {
            ...
            return "<span class=\"none\">".( 0 < $neededTime ? buildings_p_willenoughresat." ".( $neededTime )." ".time_hour_lang : buildings_p_notenoughres2 )."</span>";
        }
        return "";
    }
