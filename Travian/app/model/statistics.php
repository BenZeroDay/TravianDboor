...
LIMIT %s,%s", array($pageIndex * $pageSize, $pageSize));
		...
		public function getGeneralSummary()
		{
            ...
            $row['active_players_count'] =$this->provider->fetchScalar( "SELECT COUNT(*) FROM p_players p WHERE TIME_TO_SEC(TIMEDIFF(NOW(), p.last_login_date)) <= %s", array(
                172800
            ) );
			return $row;
		}
		...
		{
			$row = $this->provider->fetchRow("SELECT  a.id, a.%s score    FROM p_alliances a    WHERE a.name LIKE '%s%%'    LIMIT 1", array($isDefense ? "defense_points" : "attack_points", $allianceName));
