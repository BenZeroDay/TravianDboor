...
class RegisterModel extends ModelBase
{
    ...
    public function isPlayerEmailExists( $playerEmail )
    {
     ...
     public function isPlayerIpExists( $ip)
    {
        return 0 < $this->provider->fetchScalar( "SELECT COUNT(*) FROM p_players p WHERE p.last_ip='%s'", array( $ip) );
    }
    ...
    public function createNewPlayer( $playerName, $playerEmail, $playerPassword, $tribeId, $position, $villageName, $mapSize, $playerType, $villageCount = 1, $snID = 0, $ip )
    {
        $this->provider->executeQuery( "INSERT p_players  SET  tribe_id='%s',  name='%s',  pwd='%s',  email='%s',  is_active='1',  active_plus_account=0,  is_blocked=0,  registration_date=NOW(),  player_type='1',  gold_num=0,  snid=%s, last_ip=%s,  medals='0::'", array( $tribeId, $playerName, md5( $playerPassword ), $playerEmail, $playerType == PLAYERTYPE_ADMIN ? 1 : 0, intval( $snID ), $ip ) );
