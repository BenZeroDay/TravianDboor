...
class VillageModel extends ModelBase
{
    ...
    public function getVillageProtection( $villageId )
    {
        $GameMetadata = $GLOBALS['GameMetadata'];
        $protectionPeriod = intval( $GameMetadata['player_protection_period'] );
        return $this->provider->fetchScalar( "SELECT TIME_TO_SEC(TIMEDIFF(DATE_ADD(p.registration_date, INTERVAL %s SECOND), NOW())) protection_remain_sec FROM p_players p WHERE p.id=%s", array(
            $protectionPeriod,
            $villageId
        ) );
    }
