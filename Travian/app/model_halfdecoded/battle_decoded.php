...
class BattleModel extends ModelBase
{
    ...
    public function executeWarResult( $taskRow )
    {
        ...
        foreach ( $troopsArrStr as $_t )
        {
            ...
            else
            {
             ...
             $tnum += $troopsArray['cropConsumption']; //fix still needed
        }
    ...
    public function _getTroopWithPower( $troops, $troopsPower, $isAttacking, $heroLevel, $peopleCount, $wringerPower = 0, $wallPower = 0, $heroId = -1, $spyAction = FALSE )
    {
        ...
        foreach ( $troops as $tid => $tnum )
        {
            ...
            if ( $tid != 99 )
            {       //could be buggy here
                $num += $returnTroops['total_live_number'];
                $tpower  += $tpower;
                $GameMetadata['troops'][$tid]['carry_load'] += $returnTroops['total_carry_load'];
        ...
        if ( 0 < $vid )
        {
            ...
            else
            {
                ...
                if ( $ts != "" )
                {
                    ...
                    {
                        $_troops = explode( ":", $tsArrStr );
                        $_vid = explode( ":", $tsArrStr );
                        list( $_vid, $_troops ) = $_tid;
                        if ( $_vid == $invid )
                        {
