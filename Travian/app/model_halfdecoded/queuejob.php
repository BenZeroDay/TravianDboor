...
class QueueJobModel extends ModelBase
{
    ...
    public function processQueue( )
    {
        ...
        if ( $mutex->lock( ) )
        {
            ...
            if ( date( "w" ) == 5 )
            {
                $row = $this->provider->fetchRow( "SELECT gs.cur_week w1, CEIL((TO_DAYS(NOW())-TO_DAYS(gs.start_date))/1) w2 FROM g_settings gs" );
                if ( $row['w1'] < $row['w2'] )
                {
    ...
    {
        $hero_in_village_id = explode( " ", $taskRow['proc_params'] );
        $hero_troop_id = explode( " ", $taskRow['proc_params'] );
        list( $hero_troop_id, $hero_in_village_id ) = $hero_troop_id;        
    ...
    public function executeTroopUpgradeTask( $taskRow )
    {
        ...
        default :
            ...
            {
            case QS_TROOP_UPGRADE_ATTACK :
                $level = explode( " ", $taskRow['proc_params'] );
                    $tid = explode( " ", $taskRow['proc_params'] );
                list( $tid, $level ) = $tid;                
                if ( isset( $this->troopsUpgrade[$tid] ) )
                {
