...
class PrivateChatModel extends ModelBase
{
    ...
    {
        $user = $this->provider->fetchScalar( "SELECT `avatar` FROM `p_players` WHERE `id` = '%s';", array(
            $to_id
        ) );
    ...
    {
        $privatechat = $this->provider->fetchResultSet( "SELECT * FROM `privatechat` WHERE (`to_id` = '%s' AND recd = 0) ORDER BY `id` ASC;", array(
            $playerId
        ) );
        $this->provider->executeQuery( "UPDATE `privatechat` SET `recd` = 1 WHERE `to_id` = '%s' AND `recd` = 0;", array(
            $playerId
        ) );
        return $privatechat;
    }
